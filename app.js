...
*/
console.log('CaymaAsistan app v0.1');

const FREE_LIMIT = 5;
const PREMIUM_KEY = "caymaasistan_premium";
const THEME_KEY   = "caymaasistan_theme";   // 'light' | 'dark' | (unset => system)   // 'light' | 'dark' | (unset => system)
...
const DATA_FILES = [
  {name:"Ev İnterneti", path:"/public/data/ev-interneti.json"},
  {name:"Mobil Hat", path:"/public/data/mobil-hat.json"},
];
...
function lsKey(model){ return `caymaasistan_progress::${model}`; }
function selKey(model){ return `caymaasistan_selection::${model}`; }
...
function noteKey(model, stepId){ return `caymaasistan_notes::${model}::${stepId}`; }
...
function clearNotesForModel(model){
  try{
    const prefix = `caymaasistan_notes::${model}::`;
    const toDel = [];
